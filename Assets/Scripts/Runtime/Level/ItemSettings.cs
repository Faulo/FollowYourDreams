...
namespace FollowYourDreams.Level {
    [CreateAssetMenu]
    sealed class ItemSettings : ScriptableAsset, IImportable {
        [Header("Setup")]
        [SerializeField]
        GameObject prefab;
...
#endif
        static string GetAnimationName(ItemAnimation animation, int _) {
            return GetAnimationName(animation);
        }
        public static string GetAnimationName(ItemAnimation animation) {
